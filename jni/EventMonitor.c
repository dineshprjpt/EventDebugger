...
jint Java_com_is_eventdebugger_EventMonitorThread_ScanFiles( JNIEnv* env,jobject thiz ) {
...
jstring Java_com_is_eventdebugger_InputDevice_getDevPath( JNIEnv* env,jobject thiz, jint index) {
	return (*env)->NewStringUTF(env, pDevs[index].device_path);
}
jstring Java_com_is_eventdebugger_InputDevice_getDevName( JNIEnv* env,jobject thiz, jint index) {
	if (pDevs[index].device_name == NULL) return NULL;
	else return (*env)->NewStringUTF(env, pDevs[index].device_name);
}

jint Java_com_is_eventdebugger_InputDevice_OpenDev( JNIEnv* env,jobject thiz, jint index ) {
	return open_device(index);
}

jint Java_com_is_eventdebugger_InputDevice_RemoveDev( JNIEnv* env,jobject thiz, jint index ) {
	return remove_device(index);
}

jint Java_com_is_eventdebugger_InputDevice_PollDev( JNIEnv* env,jobject thiz, jint index ) {
	if (index >= nDevsCount || pDevs[index].ufds.fd == -1) return -1;
...
jint Java_com_is_eventdebugger_InputDevice_getType( JNIEnv* env,jobject thiz ) {
	return event.type;
}

jint Java_com_is_eventdebugger_InputDevice_getCode( JNIEnv* env,jobject thiz ) {
	return event.code;
}

jint Java_com_is_eventdebugger_InputDevice_getValue( JNIEnv* env,jobject thiz ) {
	return event.value;
}
